...
int is_valid_JSON(std::string filename) {
	...
	if (file.is_open()) {
		...
		while (std::getline(file, line)) { // readin a line from the file
			for (auto& ch : line) {
				if (!quotes) { // content outside quotes.
					...
					case ']':
						...
						if (stack.top() == (ch - 2)) { // here if ch is } then ch -2 corresponds to { (accroding to ASCII table. // here if ch is ] then ch - 2 corresponds to [ (accroding to ASCII table.
							stack.pop();
						}
						else {
							file.close();
